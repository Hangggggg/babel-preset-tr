...
function createChildrenExpression(types, children) {
  ...
  return arr.length ? arr.length === 1 ? arr[0] : types.arrayExpression(arr) : types.identifier('undefined');
}
...
function createHExpression(types, tagName, attributeExpression, childrenExpression) {
  return types.callExpression(
    types.identifier('Tr.createElement'), 
    [
      /^[A-Z]/.test(tagName) ? types.identifier(tagName) : types.stringLiteral(tagName), 
      attributeExpression, 
      childrenExpression,
...
function createImportTrDeclaration(types, from) {
  return types.importDeclaration([types.importDefaultSpecifier(types.identifier('Tr'))], types.stringLiteral(from));
}
